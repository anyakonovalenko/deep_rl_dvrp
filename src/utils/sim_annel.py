...
def calculate_distance(cities, solution):
    ...
    for i in solution_for_distance_calculation:  # i will hold first city indexes
        ...
        if next_city_index_founder < len(solution_for_distance_calculation):
            next_city_index = solution_for_distance_calculation[
                next_city_index_founder]  # Find the second city indexes here
            distance += np.sqrt(((cities[next_city_index, 0] - cities[i, 0]) ** 2) + ((cities[next_city_index, 1] -
                                                                                           cities[
                                                                                               i, 1]) ** 2))  # First city and second city indexes are used when calculating euclidean distance

    return distance


def generate_solution(
        current_solution):  # A new solution will be created by swapping two random cities in the current solution
...
def sim_annel_solve(dataset, T, cooling_rate, T_lower_bound, tolerance):
    ...
    while T > T_lower_bound:  # We want the algorithm to run when the temperature is greater than T_lower_bound
        ...
        while True:  # Local search will be done here; different solutions will be tried for the "same" temperature value, until new solutions give very close values ​​to the current solution (that is, until the difference in costs between the new potential solution and the current solution is less than the tolerance)
            potential_solution = generate_solution(
                current_solution)  # The potential solution is created using the generate_solution function
            potential_distance = calculate_distance(dataset,
                                                    potential_solution)  # The cost of the potential solution is calculated with the calculate_distance function
            current_distance = calculate_distance(dataset,
                                                  current_solution)  # The cost of the current solution is calculated with the calculate_distance function
            ...
            if potential_distance < current_distance:  # If the potential solution is better, the potential solution is accepted
                current_solution = potential_solution
                    ...
                    potential_distance - current_distance) / T):  # Potential solution has a chance to be accepted based on a probability even if it is worse
                current_solution = potential_solution

            if np.abs(
                    potential_distance - current_distance) < tolerance:  # Local search will run until a potential solution gives a cost value very close to the current solution
                break
...
#      1)  # (Dataset including city coordinates, Starting temperature, Cooling rate, Lower bound of temperature, Tolerance of local search) You can set these arguments as you want